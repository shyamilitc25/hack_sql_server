...
router.post('/create', async (req, res) => {
  ...
  try {
    ...
    await pool.execute('INSERT INTO admins (username, password_hash) VALUES (?, ?)', [username, hashedPassword]);
    const [[admin]] = await pool.execute('SELECT * FROM admins WHERE username = ?', [username]);
    res.json({ message: 'Admin created successfully', admin: { id: admin.id, username } });
  } catch (error) {
