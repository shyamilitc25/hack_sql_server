...
router.post('/upload', upload.single('image'), async (req, res) => {
  ...
  await pool.query('UPDATE candidates SET photo_url = ? WHERE id = ?', [req.file.filename, candidateId]);
  res.json({ message: 'Image uploaded', file: req.file.filename });
});

// Get candidate image
router.get('/:candidateId', async (req, res) => {
  const [[candidate]] = await pool.query('SELECT photo_url FROM candidates WHERE id = ?', [req.params.candidateId]);
